...
def index_songs():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    # Clear songs table to ensure no stale entries
    ...
    for playlist_dir in MUSIC_DIR.iterdir():
        if playlist_dir.is_dir():
            ...
            # Ensure playlist exists in playlists table
...
async def update_playlist_image(playlist_id: int = Form(...), image: UploadFile = File(...)):
    ...
    # Save new image with original extension
...
async def rename_playlist(playlist_id: int = Form(...), new_name: str = Form(...)):
    ...
    # Update database regardless of directory rename
